        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            {quality === 'disconnected' ? (
              ...
              <Wifi className={`h-4 w-4 ${quality === 'poor' ? 'text-orange-500' : 'text-green-500'}`} />
            )}
            <span className="text-sm font-medium">Connection Quality: {getQualityText()}</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground">Latency: {latency}ms</span>
            <span className="text-xs text-muted-foreground">Packet Loss: {packetLoss}%</span>
          </div>
        </div>
        <Progress value={getQualityProgress()} className={`h-1.5 ${getQualityColor()}`} />
      </CardContent>
    </Card>
  );
};
