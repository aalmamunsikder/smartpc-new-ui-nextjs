...
const ResourceMonitoring = () => {
  ...
  const [memoryData, setMemoryData] = React.useState(generateTimeSeriesData(60, 10));
  const [diskData, setDiskData] = React.useState(generateTimeSeriesData(25, 5));
  const [networkData, setNetworkData] = React.useState(generateTimeSeriesData(30, 15));
  ...
  React.useEffect(() => {
    ...
    const updateData = () => {
      ...
      const newPoint = (data: typeof cpuData) => {
        ...
        const newValue = Math.max(0, Math.min(100, baseValue + (Math.random() * 10 - 5)));
        return [...data.slice(1), {
    ...
    <Card className="col-span-full">
      <CardContent className="pt-6">
        ...
        <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-semibold tracking-tight">Resource Monitor</h2>
              {isLive && (
