  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Set up recurring start and stop times for your PC</CardDescription>
      </CardHeader>
      <CardContent>
              ...
              {schedules.map((schedule) => (
                <div 
                  ...
                  className={`p-3 rounded-md border flex items-center justify-between ${
                    schedule.active ? 'border-primary/50 bg-primary/5' : 'border-muted'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    {schedule.type === 'start' ? (
                      <Power className={`h-4 w-4 ${schedule.active ? 'text-green-500' : 'text-muted-foreground'}`} />
                    ) : (
                      <PowerOff className={`h-4 w-4 ${schedule.active ? 'text-red-500' : 'text-muted-foreground'}`} />
                    )}
                    <div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-4">
                    <div className="flex items-center space-x-2">
                      ...
                      />
                      <Label htmlFor="action-start" className="flex items-center gap-1">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      />
                      <Label htmlFor="action-stop" className="flex items-center gap-1">
                    ...
                    {Object.entries(editItem.days).map(([day, checked]) => (
                      <div key={day} className="flex flex-col items-center gap-1.5">
                        ...
                        <Checkbox 
                          id={`day-${day}`}
                          checked={checked}
                          onCheckedChange={() => handleUpdateDay(day as keyof ScheduleItem['days'])}
                        />
                      </div>
                    ))}
                  </div>
