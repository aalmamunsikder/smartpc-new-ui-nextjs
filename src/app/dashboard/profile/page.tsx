...
import { Camera, Mail, Key, Shield, AlertTriangle, Smartphone, QrCode } from 'lucide-react';
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {sessions.map(session => (
                <div key={session.id} className="flex items-start justify-between border-b last:border-0 pb-4 last:pb-0">
                  <div className="space-y-1">
                    <p className="font-medium">{session.device}</p>
                    <p className="text-xs text-muted-foreground">
                      {session.location} • IP: {session.ip}
                    </p>
                    <p className={`text-xs ${session.isCurrentSession ? "text-primary" : ""}`}>
                      {session.lastActive}
                    </p>
                  </div>
      ...
      <Dialog open={show2FADialog} onOpenChange={setShow2FADialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {mfaMethod === 'app' && "Scan the QR code below with your authenticator app"}
              {mfaMethod === 'sms' && "Enter your phone number to receive verification codes"}
              {mfaMethod === 'email' && "We'll send verification codes to your email"}
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Input 
                      ...
                      className={!isPhoneValid && phoneNumber ? "border-destructive" : ""}
                    />
                    <Button
                      type="button"
                      variant="secondary"
                    ...
                    >
                      {cooldown > 0 ? `Resend (${cooldown}s)` : isCodeSent ? "Resend" : "Send Code"}
                    </Button>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Input 
                      ...
                      className={!isEmailValid && preferredEmail ? "border-destructive" : ""}
                    />
                    <Button
                      type="button"
                      variant="secondary"
